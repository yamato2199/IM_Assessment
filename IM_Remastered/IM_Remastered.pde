...
float keyWidth; 
ArrayList<PianoKey> pressedKeys = new ArrayList<PianoKey>();
...
void draw(){
   ...
   trackBlocks(video,color(0,0,255),130);
...
int trackBlocks(Capture video,color trackColor,float threshold) {
    int currentY = 0;
    ...
    for (int y = 0; y < video.height; y++ ) {
      ...
      if (d < threshold*threshold) {
        ...
        if(!find && blocks.size() < Config.MAX_BLOCKS){
...
void setupPianoKey(){
  ...
  for (int i=0; i<whiteKeys.length; i++) {
    whiteKeys[i]=new PianoKey(new PVector((keyWidth*144/24/2)-200, (keyWidth/2+i*keyWidth)-20), 
      color(#FFFFFF), keyWidth*144/24, keyWidth, 101-i);
  }

  // Initial black keys
  for (int i=0; i<blackKeys.length; i++) {
    blackKeys[i]=new PianoKey(new PVector((keyWidth*86/24/2)-100, ((i+1)*keyWidth)-5), 
...
void triggerKeys(){
    //Find all triggered points from blocks
    for(Block b : blocks){
      triggerhitKeys((int)b.position().y);
    }
}
